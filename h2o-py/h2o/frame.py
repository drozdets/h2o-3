...
class H2OFrame(object):
    ...
    def diff(self,lag):
        """Conduct a lag transform on a numeric H2OFrame column

        Returns
        -------
          H2OFrame column with lag a transform
        """
        fr = H2OFrame._expr(expr=ExprNode("diff", self,lag), cache=self._ex._cache)
        return fr
